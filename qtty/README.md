...
- `serde`: serializes/deserializes `Quantity<U>` as bare `f64` values (unit is encoded by the type).
 - `python`: enables PyO3 conversions so `Quantity<U>` types convert to/from Python `float` and can be used
	 directly as `#[pyclass]` fields (requires building with `--features python` for `qtty-core`).
